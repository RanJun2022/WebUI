...
function isChinese(str) {
    let cnChar = str.match(/[^\x00-\x80]/g);
...
function isEnglish(str) {
    let cnChar = str.match(/^[a-zA-Z\s]+$/g);
    if (cnChar !== null && cnChar.length > 0)
        return true;
    return false;
}
...
function arraySearch(str) {
    for (let name in PinYinLibs) {
        if (PinYinLibs[name].indexOf(str) !== -1) {
            return name;
        }
    }
    return false;
...
function getPinYinAndFirstCharacter(str) {
    if (!str) {
        return '';
    }
    let len = str.length;
    ...
    for (let i = 0; i < len; i++) {
        ...
        if (isChinese(val)) {
            name = arraySearch(val);
            if (reg.test(val)) {
                firstResult.push(val);
                result.push(val);
            } else if (name !== false) {
                firstResult.push(name.substring(0, 1));
                result.push(name);
            }
        } else {
            firstResult.push(val);
    ...
    return {first:firstResult, py:result};
}

const PinYin = {
    isChinese: isChinese,
    getPinYin: getPinYin,
    getPinYinFirstCharacter: getPinYinFirstCharacter,
